...
from lgm.core.options import config_defaults, Options
...
class Converter(nn.Module):
    def __init__(self, device, opt: Options, plydata):
        super().__init__()
        ...
        self.gaussians = self.gs_renderer.load_ply(plydata).to(self.device)
        ...
        self.encoder_density = GridEncoder(num_levels=24)  # VMEncoder(output_dim=16, mode='sum')
        self.encoder = GridEncoder(num_levels=24)
    ...
    def render_gs(self, pose):
        ...
        out = self.gs_renderer.render(self.gaussians.unsqueeze(0), cam_view.unsqueeze(0), cam_view_proj.unsqueeze(0),
                                      cam_pos.unsqueeze(0))
    ...
    def render_nerf(self, pose):
        ...
        weights, trans, alphas = nerfacc.render_weight_from_density(t_starts, t_ends, sigmas, ray_indices=ray_indices,
                                                                    n_rays=n_rays)
    ...
    def render_mesh(self, pose):
        ...
        v_cam = torch.matmul(F.pad(v, pad=(0, 1), mode='constant', value=1.0), torch.inverse(pose).T).float().unsqueeze(
            0)
        ...
        alpha = dr.antialias(alpha, rast, v_clip, f).clamp(0, 1).squeeze(-1).squeeze(
            0)  # [H, W] important to enable gradients!
    ...
    def fit_mesh(self, iters=2048, resolution=512, decimate_target=4e4):
            ...
            for yi, ys in enumerate(Y):
                for zi, zs in enumerate(Z):
                    ...
                    sigmas[xi * S: xi * S + len(xs), yi * S: yi * S + len(ys), zi * S: zi * S + len(zs)] = val.reshape(
                        len(xs), len(ys), len(zs)).detach().cpu().numpy()  # [S, 1] --> [x, y, z]
        ...
        vertices = self.laplacian_smooth(vertices, triangles, num_iterations=3)
        self.v = torch.from_numpy(vertices).contiguous().float().to(self.device)
        self.f = torch.from_numpy(triangles).contiguous().int().to(self.device)
        self.deform = nn.Parameter(torch.zeros_like(self.v).to(self.device))
    ...
    # uv mesh refine
    def fit_mesh_uv(self, iters=512, resolution=512, texture_resolution=512, padding=2):
        ...
        # Optimizer for texture and super-resolution network
        optimizer = torch.optim.Adam([
        ...
        for i in pbar:
            ...
            # Calculate loss
            loss_mse = nn.functional.mse_loss(image_pred, image_gt)
            loss_sharp = self.texture_sharpness_loss(self.albedo)
            loss_total = loss_mse + 2 * loss_sharp
            ...
            pbar.set_description(f"MSE = {loss_mse.item():.6f}, Sharp = {loss_sharp.item():.6f}")

        print(f"[INFO] finished fitting mesh texture!")

    @staticmethod
    def texture_sharpness_loss(texture, kernel_size=3):
        # Compute texture gradients
        grad_x = texture[:, 1:, :] - texture[:, :-1, :]
        grad_y = texture[1:, :, :] - texture[:-1, :, :]

        # Compute sharpness loss
        loss = torch.mean(torch.abs(grad_x)) + torch.mean(torch.abs(grad_y))

        # Apply Laplacian filter for sharpening
        kernel = torch.tensor([[0, -1, 0], [-1, 4, -1], [0, -1, 0]], dtype=torch.float32, device=texture.device)
        kernel = kernel.view(1, 1, kernel_size, kernel_size)

        sharpened_texture = torch.zeros_like(texture)
        for c in range(texture.shape[2]):
            sharpened_texture[:, :, c] = nn.functional.conv2d(texture[:, :, c].unsqueeze(0).unsqueeze(0), kernel, padding=kernel_size // 2).squeeze(0).squeeze(0)

        # Compute sharpness loss on the sharpened texture
        sharp_loss = torch.mean(torch.abs(sharpened_texture - texture))

        return loss * sharp_loss

    def rotate_mesh(self, mesh):
        # Rotate mesh to align to view
        """Rotate the mesh around the X-axis by the given angle in degrees."""
        ...
        # Apply the rotation matrix to vertex positions and normals
        mesh.v = torch.matmul(mesh.v, rot_matrix)
        if mesh.vn is not None:
            mesh.vn = torch.matmul(mesh.vn, rot_matrix)
    ...
    def export_mesh(self, output_path):
        ...
        self.rotate_mesh(mesh)

        mesh.write(output_path)


def convert_gs_to_mesh(device, gs, output_path):
    opt = config_defaults['big']

    converter = Converter(device, opt, gs).cuda()
    converter.fit_nerf()
    converter.fit_mesh(iters=opt.mesh_iters)
    converter.fit_mesh_uv()
    converter.export_mesh(output_path)
