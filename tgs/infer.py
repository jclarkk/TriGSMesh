...
from tgs.models.renderer import inverse_sigmoid
    ...
    def forward(self, batch):
        ...
        for b in range(batch_size):
            if batch["view_index"][b, 0] == 0:
                self.gs_output = self.normalize_to_ply(out["3dgs"][b])
                self.save_ply = out["3dgs"][b].save_ply

    @staticmethod
    def normalize_to_ply(gs):
        xyz = gs.xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)
        features_dc = gs.shs[:, :1]
        features_rest = gs.shs[:, 1:]
        ...
        opacities = inverse_sigmoid(torch.clamp(gs.opacity, 1e-3, 1 - 1e-3).detach().cpu().numpy())
        scale = np.log(gs.scaling.detach().cpu().numpy())
        rotation = gs.rotation.detach().cpu().numpy()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_preprocess", action="store_true",
                        help="whether to segment the input image by rembg and SAM")
    ...
    model_path = hf_hub_download(repo_id="VAST-AI/TriplaneGaussian", local_dir="./checkpoints",
                                 filename="model_lvis_rel.ckpt", repo_type="model")
    ...
    if args.image_preprocess:
        ...
        for image_path in cfg.data.image_list:
            ...
            subprocess.run([f"python image_preprocess/run_sam.py --image_path {image_path} --save_path {save_path}"],
                           shell=True)
